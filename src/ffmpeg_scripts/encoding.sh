...
review_enc="-c:v hevc_videotoolbox -profile:v 0 -tag:v hvc1 -b:v 50M -c:a aac -pix_fmt nv12 -colorspace bt709 -color_primaries bt709 -color_trc bt709 -dst_range 1 -src_range 1 -color_range 2 -sws_flags +accurate_rnd+full_chroma_int+full_chroma_inp -sws_dither none -sar 1:1 -fflags +igndts -fflags +genpts -vsync 1 -r ${r} -threads ${threads} -g ${r}"
